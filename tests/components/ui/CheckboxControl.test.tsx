...
describe('CheckboxControl', () => {
  ...
  it('reduces opacity and removes press feedback when disabled', () => {
    const view = renderWithTheme(
      <CheckboxControl checked disabled onCheckedChange={jest.fn()} />,
      { defaultTheme: 'dark' },
    )

    const checkbox = view.UNSAFE_getByType(Checkbox)

    expect(checkbox.props.opacity).toBe(0.5)
    expect(checkbox.props.pressStyle).toBeUndefined()
  })
})
